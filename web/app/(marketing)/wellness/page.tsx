...
export default function WellnessPage() {
  ...
  // Auto-detect city via IP on mount
  useEffect(() => {
    fetch('https://ipapi.co/json/')
      ...
      .catch(() => console.warn('City detection failed — fallback to manual'));
  }, []);
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      await waitlistService.join({ name, email, city, source: 'wellness_page' });
      toast.success('Joined waitlist! Check your mail.');
      setName('');
      setEmail('');
    } catch (error: unknown) {
      const e = error as { response?: { data?: { message?: string } }; message?: string };
      toast.error(e.response?.data?.message || e.message || 'Failed to join waitlist. Please try again.');
    } finally {
        ...
        <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left mt-8 md:mt-0">
          ...
          <div className="w-full max-w-md flex flex-col gap-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Joining...</> : 'Join Waitlist'}
              </button>
            </div>

            <p className="text-sm text-gray-400 mt-2">Be the first to access our new features.</p>
          </div>

        </div>

        {/* Sneak Peek Mockups (remains same) */}
