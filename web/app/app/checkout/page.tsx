...
function CheckoutContent() {
  ...
  const [createdReferenceId, setCreatedReferenceId] = useState<string | null>(
    null,
  );
  ...
  const cancelPaymentOnBackend = async () => {
    if (pendingOrderId.current) {
      try {
        ...
        console.warn("cancelPayment non-blocking error:", e);
      }
    ...
    try {
      ...
      if (!referenceId) {
        if (isProduct) {
          ...
          const orderRes = await apiClient.post<{ id: string }>(
            `/products/${productItem.productId}/order`,
            { quantity: 1 },
          );
          ...
          const bookingPromises = items.map((item) => {
            const rawDate =
              (item.scheduleTime as string) ||
              ((item as Record<string, unknown>).selectedDate as string) ||
              "";
            const finalScheduledDate =
              rawDate && rawDate !== "ASAP"
                ? rawDate.replace(" at ", " ")
      ...
      if (paymentMethod === "cash") {
        toast.success(
          "Confirmed! Please pay ₹" + formatPrice(total) + " upon arrival.",
          { icon: "⌛" },
        );
        ...
        const options: Record<string, unknown> = {
          ...
          description: isSubscription
            ...
            : isProduct
              ? `Wellness Store Order`
              : `Payment for ${items.length} service(s)`,
          order_id: orderId,
          ...
          config: {
            display: {
              blocks: {
                [paymentMethod]: {
                  name:
                    paymentMethod === "upi" ? "UPI Options" : "Card Options",
            ...
            try {
              ...
              if (verifyRes.success) {
                ...
                const { USER_QUERY_KEYS } =
                  await import("@/hooks/useUserHooks");
        ...
        rzp.on(
          ...
          async function (response: Record<string, unknown>) {
            ...
            const rzpErr = response.error as
              | { description?: string }
              | undefined;
    ...
    <div className="flex flex-col min-h-screen bg-[#f8f9fc] pb-32">
      ...
      <div className="px-4 flex flex-col gap-6 max-w-xl mx-auto w-full text-center">
        ...
        <div className="text-left">
          <h2 className="text-md font-bold text-gray-900 mb-4 px-1">
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center ${paymentMethod === "upi" ? "bg-emerald-500 text-white" : "bg-indigo-50 text-indigo-600"}`}
              >
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-500 mt-0.5">
                  Instant activation via GPay, PhonePe, etc.
                </div>
              </div>
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${paymentMethod === "upi" ? "border-emerald-500 bg-emerald-500" : "border-gray-300"}`}
              >
                {paymentMethod === "upi" && (
                  <div className="w-2 rounded-full border border-white" />
                )}
              </div>
            </button>
            ...
            >
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center ${paymentMethod === "card" ? "bg-emerald-500 text-white" : "bg-slate-50 text-slate-600"}`}
              >
              ...
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${paymentMethod === "card" ? "border-emerald-500 bg-emerald-500" : "border-gray-300"}`}
              >
                {paymentMethod === "card" && (
                  <div className="w-2 rounded-full border border-white" />
                )}
            ...
            {!isSubscription && (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-black ${paymentMethod === "cash" ? "bg-amber-600 text-white" : "bg-amber-50 text-amber-600"}`}
                >
                  ₹
                </div>
                <div className="flex-1">
                ...
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${paymentMethod === "cash" ? "border-amber-600 bg-amber-600" : "border-gray-300"}`}
                >
                  {paymentMethod === "cash" && (
                    <div className="w-2 rounded-full border border-white" />
                  )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-6 pb-10 md:bottom-8 md:max-w-xl md:mx-auto md:rounded-3xl shadow-2xl z-20">
        <button
          ...
          disabled={isProcessing}
          className="w-full h-15 bg-gray-900 text-white rounded-2xl font-black text-lg flex items-center justify-center transition-all active:scale-95 hover:bg-black shadow-xl disabled:opacity-50"
        >
          {isProcessing ? (
