...
import { ChevronLeft, Calendar as CalendarIcon, Clock, MapPin, CheckCircle } from 'lucide-react';
              ...
              {PROBLEMS.map((item) => (
                <button
                  ...
                  className={`flex flex-col items-center bg-white rounded-xl shadow-sm transition-all overflow-hidden border-2 p-1 ${
                    selectedProblem === item.id ? 'border-[var(--color-primary)] ring-4 ring-[var(--color-primary)]/5' : 'border-transparent'
                  }`}
                >
        ...
        {step === 4 && (
          <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-6">
             ...
             <button 
               ...
               className="mt-8 bg-[var(--color-primary)] text-white h-14 rounded-2xl font-bold shadow-lg shadow-[var(--color-primary)]/30 flex items-center justify-center gap-2 active:scale-95 transition-transform"
             >
