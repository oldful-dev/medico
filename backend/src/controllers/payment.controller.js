...
const verifyPayment = async (req, res, next) => {
    try {
        ...
        const result = await processPaymentSuccess(orderId, paymentId, signature, paymentMethod);

        if (!result) {
            return res.status(400).json({ success: false, message: 'Failed to process payment verification' });
        }

        sendResponse(res, 200, { payment: result.payment, invoice: result.invoice }, 'Payment verified successfully');
    } catch (error) {
        next(error);
    }
};
