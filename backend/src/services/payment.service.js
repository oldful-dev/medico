...
const processPaymentSuccess = async (orderId, paymentId, signature, paymentMethod) => {
    // 1. Double check status to avoid starting transaction if already success
    const paymentCheck = await prisma.payment.findUnique({
        where: { razorpayOrderId: orderId },
        include: { invoice: true }
    });

    if (paymentCheck && paymentCheck.status === 'SUCCESS') {
        logger.info(`[PaymentService] Order ${orderId} already marked as SUCCESS. Skipping promotion.`);
        return { payment: paymentCheck, invoice: paymentCheck.invoice };
    }

    // 2. Atomic update to ensure single-winner transition
    ...
    if (updateResult.count === 0) {
        logger.info(`[PaymentService] Order ${orderId} transitioned by concurrent task. Skipping promotion.`);
        const recheck = await prisma.payment.findUnique({
            where: { razorpayOrderId: orderId },
            include: { invoice: true }
        });
        return { payment: recheck, invoice: recheck?.invoice };
    }

    logger.info(`[PaymentService] Successfully transitioned order ${orderId} to SUCCESS. Executing promotions...`);
    ...
    const txResult = await prisma.$transaction(async (tx) => {
        ...
        return { payment: paymentRecord, subscription: activeSub, invoice };
    });

    // 4. Load full consolidated payment details
    ...
    // 5. IMMEDIATE: Real-time Admin WebSocket Notifications (fire before any async work)
    try {
        if (payment?.booking) {
    ...
    // 6. Heavy lifting (Non-blocking) — PDF, email, WA, SMS, Delhivery
    setImmediate(async () => {
        try {
