...
export default function HelpSupportLandingScreen() {
    ...
    const handleCall = () => Linking.openURL(`tel:${helpSupportConfig.phone}`);
    ...
    return (
        <View style={styles.screen}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>{t('help_support.header')}</Text>
            </View>

            <View style={styles.contentCard}>
                <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                    <Text style={styles.pageDescription}>
                        {getText(helpSupportConfig.page_description)}
                    </Text>

                    {/* Contact Options */}
                    <Text style={styles.sectionTitle}>{t('help_support.contact_options')}</Text>
                    ...
                    <TouchableOpacity style={styles.contactCard} activeOpacity={0.7} onPress={handleCall}>
                        ...
                        <View style={styles.contactTextGroup}>
                            <Text style={styles.contactTitle}>{t('help_support.call_us')}</Text>
                            <Text style={styles.contactDesc}>{t('help_support.call_us_desc')}</Text>
                        </View>
                        <Ionicons name="chevron-forward" size={20} color={colors.textMuted} />
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.contactCard} activeOpacity={0.7} onPress={() => router.push('/help-support/chat')}>
                        <View style={styles.iconBoxWhatsapp}>
                            <Ionicons name="logo-whatsapp" size={24} color={colors.textWhite} />
                        </View>
                        <View style={styles.contactTextGroup}>
                            <Text style={styles.contactTitle}>{t('help_support.whatsapp_us')}</Text>
                            <Text style={styles.contactDesc}>{t('help_support.whatsapp_us_desc')}</Text>
                        </View>
                        <Ionicons name="chevron-forward" size={20} color={colors.textMuted} />
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.contactCard} activeOpacity={0.7} onPress={() => router.push('/help-support/raise-ticket')}>
                        <View style={styles.iconBoxTicket}>
                            <Ionicons name="chatbubbles" size={22} color={colors.textWhite} />
                        </View>
                        <View style={styles.contactTextGroup}>
                            <Text style={styles.contactTitle}>{t('help_support.raise_ticket')}</Text>
                            <Text style={styles.contactDesc}>{t('help_support.raise_ticket_desc')}</Text>
                        </View>
                        <Ionicons name="chevron-forward" size={20} color={colors.textMuted} />
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.contactCard} activeOpacity={0.7} onPress={() => router.push('/help-support/faq')}>
                        <View style={styles.iconBoxFAQ}>
                            <Ionicons name="help-circle" size={24} color={colors.textWhite} />
                        </View>
                        <View style={styles.contactTextGroup}>
                            <Text style={styles.contactTitle}>{t('help_support.faqs')}</Text>
                            <Text style={styles.contactDesc}>{t('account.faq_subtitle')}</Text>
                        </View>
                        <Ionicons name="chevron-forward" size={20} color={colors.textMuted} />
                    </TouchableOpacity>
                    ...
                    {/* Support Contacts */}
                    <Text style={styles.sectionTitle}>{t('help_support.support_contact')}</Text>
                    <View style={styles.infoCard}>
                        {helpSupportConfig.contacts.map((contact, i) => (
                            <React.Fragment key={contact.id}>
                                {i > 0 && <View style={styles.divider} />}
                                <Text style={styles.contactHeader}>{getText(contact.label)}</Text>
                                {contact.name  && <Text style={styles.contactDetail}>Name: {getText(contact.name)}</Text>}
...
const makeStyles = (colors: ThemeColors, isDarkMode: boolean) => StyleSheet.create({
    ...
    iconBoxFAQ:      { width: 44, height: 44, borderRadius: 22, backgroundColor: '#2E7D32', justifyContent: 'center', alignItems: 'center', marginRight: Spacing.md },
