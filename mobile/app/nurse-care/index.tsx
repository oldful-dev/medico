...
import {
  familyMemberService,
  FamilyMember,
} from "@/services/api/familyMemberService";
...
export default function BookNursingCareScreen() {
  ...
  const handleAddFamilyMember = async () => {
    ...
    if (!newMemberName.trim()) {
      triggerAlert(
        t("family_member_form.required_title"),
        t("family_member_form.name_required_msg"),
      );
      return;
    }
    try {
      ...
      if (res.success && res.data) {
        triggerAlert(
          t("family_member_form.success_title"),
          t("family_member_form.member_added_msg"),
        ...
        triggerAlert(
          t("family_member_form.error_title"),
          res.message || t("family_member_form.add_failed_msg"),
        );
      }
    } catch (error) {
      ...
      triggerAlert(
        t("family_member_form.error_title"),
        t("family_member_form.add_failed_msg"),
      );
    }
  };
  ...
  // Mirrors backend/src/controllers/checkout.controller.js's resolveOptionPrice
  // exactly (id match, then label-substring match against the same literal
  // string checkout will receive as selectedOption) — the mobile duration-card
  // price and the price checkout actually charges must agree, so both sides
  // now run identical matching instead of two independently-drifting parsers.
  const resolveOptionPrice = React.useCallback(
    (selectedOption: string): number | null => {
      const sections = dbService?.formFieldsJson?.sections;
      if (!Array.isArray(sections)) return null;
      const needle = selectedOption.toLowerCase();
      for (const section of sections) {
        for (const field of section?.fields || []) {
          const options = field?.options || [];
          const byId = options.find((o: any) => o?.id === selectedOption);
  ...
  // Parse duration prices — resolveOptionPrice first (matches what checkout
  // will actually charge), falling back to basePrice/pricingText/defaults
  // when admin hasn't configured per-duration options.
  const durationPrices = React.useMemo(() => {
    let short = 499;
    let full = 1299;
    const addon = 200;

    const shortMatch = resolveOptionPrice("Short Visit (2 Hours)");
    const fullMatch = resolveOptionPrice("Full Shift (8 Hours)");
    if (shortMatch != null) short = shortMatch;
    if (fullMatch != null) full = fullMatch;

    if (shortMatch == null && fullMatch == null) {
  ...
  const [selectedAddress, setSelectedAddress] =
    React.useState<AddressData | null>(
      activeAddress
        ? {
            id: activeAddress.id,
  ...
  const isFormValid = React.useMemo(() => {
    return !!(
      ...
      selectedAddress.line1.trim().length >= 5
    );
  }, [
    selectedWho,
  ...
  const handleBookService = async () => {
    ...
    if (!selectedAddress?.line1 || selectedAddress.line1.trim().length < 5) {
      triggerAlert(
        t("nurse_care.alert_address_required"),
    ...
    if (selectedDuration.includes("Short")) shiftDuration = "SHORT_VISIT";
      ...
      selectedDuration.includes("8") ||
      selectedDuration.includes("Full")
    )
      shiftDuration = "TWELVE_HOUR";
    else if (selectedDuration.includes("12")) shiftDuration = "TWELVE_HOUR";
    else if (selectedDuration.includes("24"))
      shiftDuration = "TWENTY_FOUR_HOUR";
    ...
    try {
      ...
      const addressLine = [selectedAddress.line1, selectedAddress.line2]
        .filter(Boolean)
        .join(", ");
      ...
      const bookingPayload = JSON.stringify({
        ...
        formDataJson: {
          recipient:
            selectedWho === "Self"
              ? "Self"
              : selectedFamilyMember?.name || "Family",
          recipientRelation:
            selectedWho === "Self"
              ? "Self"
      ...
      // Same resolveOptionPrice values the duration cards displayed — never
      // a second, independently-matched guess at what checkout will charge.
      const calculatedPrice =
        selectedDuration === "Full Shift (8 Hours)"
          ? durationPrices.full
          : durationPrices.short;
      ...
      router.push({
        ...
        params: {
          ...
          ...(dbService?.checkoutGroup && {
            checkoutGroup: dbService.checkoutGroup,
          }),
            ...
            >
              ...
              <View style={dynamicStyles.sectionContainerBase}>
                <Text style={dynamicStyles.sectionTitle}>
                  {t("nurse_care.who_for")}
                </Text>
                <View style={dynamicStyles.whoRow}>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.whoButton,
                      selectedWho === "Self" && dynamicStyles.whoButtonActive,
                    ]}
                  ...
                  >
                    {selectedWho === "Self" && (
                      <Ionicons
                        ...
                        size={16}
                        color="#02743F"
                        style={dynamicStyles.whoIconCheck}
                      />
                    )}
                    <Text style={dynamicStyles.whoButtonText}>
                      {t("common.self")}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.whoButton,
                      selectedWho === "Family" && dynamicStyles.whoButtonActive,
                    ]}
                    onPress={() => setSelectedWho("Family")}
                  >
                    <Image
                      source={familyIcon}
                      style={dynamicStyles.whoIcon}
                      resizeMode="contain"
                    />
                    <Text style={dynamicStyles.whoButtonText}>
                      {t("care_form.family")}
                ...
                {selectedWho === "Family" && (
                  <View style={{ marginTop: 12 }}>
                    <Text style={dynamicStyles.subSectionTitle}>
                      {t("care_form.select_family_member")}
                    </Text>
                          ...
                          <TouchableOpacity
                            ...
                            style={[
                              ...
                              selectedFamilyMemberId === member.id &&
                                dynamicStyles.familyChipActive,
                            ]}
                          ...
                          >
                            <Text
                              style={[
                                ...
                                selectedFamilyMemberId === member.id &&
                                  dynamicStyles.familyChipTextActive,
                              ]}
                            >
                    ...
                    >
                      ...
                      <Text style={dynamicStyles.addFamilyBtnText}>
                        {t("family_members.add")}
                      </Text>
                    </TouchableOpacity>
                  </View>
                )}
                ...
                {selectedWho === "Family" && showAddFamilyModal && (
                  <View style={dynamicStyles.addFamilyForm}>
                    <Text style={dynamicStyles.formLabel}>
                      {t("family_member_form.full_name")}
                    </Text>
                    <TextInput
                      style={dynamicStyles.formInputInline}
                      placeholder={t(
                        "family_member_form.enter_name_placeholder",
                      )}
                      value={newMemberName}
                        ...
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            newMemberRelation === rel &&
                              dynamicStyles.relationChipActive,
                          ]}
                        ...
                        >
                          <Text
                            style={[
                              ...
                              newMemberRelation === rel &&
                                dynamicStyles.relationChipTextActive,
                            ]}
                          >
                    ...
                    <View style={dynamicStyles.formActionRow}>
                      ...
                      >
                        <Text style={dynamicStyles.cancelFormBtnText}>
                          {t("family_member_form.cancel")}
                        </Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                      ...
                      >
                        <Text style={dynamicStyles.saveFormBtnText}>
                          {t("family_member_form.save")}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  </View>
              ...
              <View style={dynamicStyles.sectionContainer}>
                <Text style={dynamicStyles.sectionTitle}>
                  {t("nurse_care.duration")}
                </Text>

                <View style={dynamicStyles.verticalStack}>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.durationCardStacked,
                      selectedDuration === "Short Visit (2 Hours)" &&
                        dynamicStyles.cardActive,
                    ]}
                  ...
                  >
                    ...
                    >
                      <Text style={dynamicStyles.radioLabelStacked}>
                        Short Visit (2 Hours)
                      </Text>
                      <View
                        style={{
              ...
              <View style={dynamicStyles.sectionContainer}>
                <Text style={dynamicStyles.sectionTitle}>
                  {t("nurse_care.condition")}
                </Text>

                <View style={dynamicStyles.gridRow}>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.radioCard,
                      selectedCondition === "Walking/ Mobile" &&
                        dynamicStyles.cardActive,
                    ]}
                    onPress={() => setSelectedCondition("Walking/ Mobile")}
                  >
                    <Ionicons
                      name={
                        selectedCondition === "Walking/ Mobile"
                    ...
                    />
                    <Text style={dynamicStyles.radioLabel}>
                      {t("nurse_care.walking")}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.radioCard,
                      selectedCondition === "Bedridden" &&
                        dynamicStyles.cardActive,
                    ]}
                    onPress={() => setSelectedCondition("Bedridden")}
                  >
                    <Ionicons
                      name={
                        selectedCondition === "Bedridden"
                ...
                <View style={dynamicStyles.gridRow}>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.radioCard,
                      selectedCondition === "Post-Surgery" &&
                        dynamicStyles.cardActive,
                    ]}
                    onPress={() => setSelectedCondition("Post-Surgery")}
                  >
                    <Ionicons
                      name={
                        selectedCondition === "Post-Surgery"
              ...
              <View style={dynamicStyles.sectionContainer}>
                <Text style={dynamicStyles.sectionTitle}>
                  {t("nurse_care.gender_pref")}
                </Text>

                <View style={dynamicStyles.gridRow}>
                  <TouchableOpacity
                    style={[
                      dynamicStyles.radioCardSmall,
                      selectedGender === "Male" && dynamicStyles.cardActive,
                    ]}
                  ...
                  >
                    <Ionicons
                      ...
                      size={16}
                      color={selectedGender === "Male" ? "#02743F" : "#AAAEAC"}
                    />
                    <Text style={dynamicStyles.radioLabel}>
                      {t("common.male")}
                    </Text>
                  ...
                  <TouchableOpacity
                    style={[
                      dynamicStyles.radioCardSmall,
                      selectedGender === "Female" && dynamicStyles.cardActive,
                    ]}
                  ...
                  >
                    <Ionicons
                      ...
                      size={16}
                      color={
                        selectedGender === "Female" ? "#02743F" : "#AAAEAC"
                      }
                    />
                    <Text style={dynamicStyles.radioLabel}>
                      {t("common.female")}
                  ...
                  <TouchableOpacity
                    style={[
                      dynamicStyles.radioCardSmall,
                      selectedGender === "Any" && dynamicStyles.cardActive,
                    ]}
                    onPress={() => setSelectedGender("Any")}
                  >
                    <Ionicons
                      name={
                        selectedGender === "Any"
                      ...
                      size={16}
                      color={selectedGender === "Any" ? "#02743F" : "#AAAEAC"}
                    />
                    <Text style={dynamicStyles.radioLabel}>
                      {t("common.any")}
                    </Text>
              ...
              {/* ─── Not Sure Banner ─── */}
              <TouchableOpacity
                style={dynamicStyles.notSureBanner}
                onPress={() => Linking.openURL("tel:08047280789")}
                activeOpacity={0.75}
              >
                <Image
                  source={helpIcon}
                  style={dynamicStyles.ideaIcon}
                  resizeMode="contain"
                />
                <View style={dynamicStyles.notSureTextGroup}>
                  <Text style={dynamicStyles.notSureTitle}>
                    {t("nurse_care.not_sure_title")}
                  </Text>
                  <Text style={dynamicStyles.notSureSubtitle}>
                    {t("nurse_care.not_sure_subtitle")}
                  </Text>
                </View>
              </TouchableOpacity>
          ...
          >
            <TouchableOpacity
              style={[
                ...
                (!isFormValid || isBooking || isLoadingInit) && {
                  opacity: 0.6,
                },
              ]}
              activeOpacity={
                isFormValid && !isBooking && !isLoadingInit ? 0.8 : 0.5
              }
  ...
  StyleSheet.create({
    ...
    bottomBarContainer: {
      ...
      borderTopColor: isDarkMode
        ...
        : "rgba(0,0,0,0.05)", // Adds a tiny visual separation line from the scroll view
