...
export const HOME_CONFIG_FALLBACK: HomeConfig = {
    ...
    sections: [
        ...
        },
        {
            ...
            services: [
                // Routes point at the shared dynamic catch-all + the service's
                // CURRENT DB slug (not the old dedicated-route names) — these
                // slugs drift when admin renames a service, so verify against
                // the live Service table (scripts/verify-mobile-service-slugs.js)
                // before trusting this list again.
                // 1. Bill Payment
